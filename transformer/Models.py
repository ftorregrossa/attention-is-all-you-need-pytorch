...
import numpy as np
import aiaynpt.transformer.Constants as Constants
from aiaynpt.transformer.Modules import BottleLinear as Linear
from aiaynpt.transformer.Layers import EncoderLayer, DecoderLayer
...
class RawEncoder(nn.Module):
    '''Encoder not using dictionary but features extracted from raw data'''
    ...
    def forward(self, src, src_pos, return_attns=False):
        '''Front end using Convolutional'''

        pos_enc = self.position_enc(src_pos)

        enc_input =  src + pos_enc
...
class RawTransformer(nn.Module):
    ...
    def __init__(
            self, enc_max_seq, n_tgt_vocab, n_max_seq, n_layers=6, n_head=8,
            d_word_vec=512, d_model=512, d_inner_hid=1024, d_k=64, d_v=64,
